...
def kernelEC(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, k_opt=300):
...
def cvEC(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T):
    ...
    k_range = np.linspace(5, min(np.floor(n_front/3), 200), 10).astype(int)
    n_k = k_range.shape[0]
    cv_score = np.zeros(n_k)
...
def kernelGA(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, k_opt=300):
...
def kernelBU(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, H, k_opt=300):
...
def kernelBD(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, H, k_opt=300):
    ...
    X_train = methods.generate_basis(sample_outer, option_type="Barrier_D", basis_function="None",
                                     sample_min=min_outer[:, :, -1])
    y_train = np.sum(payoff, axis=0)
    ...
    X_test = methods.generate_basis(sample_outer_test, option_type="Barrier_D", basis_function="None",
...
def calRM(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h=3/50, H=120, L0=100, 
          optionName="European", k_opt=300, level_list=[0.8, 0.9, 0.95, 0.99, 0.996]):

    if optionName == "European":
        loss = kernelEC(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, k_opt)
    elif optionName == "Asian":
        loss = kernelGA(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, k_opt)
    elif optionName == "BarrierUp":
        loss = kernelBU(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, H, k_opt)
    elif optionName == "BarrierDown":
        loss = kernelBD(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, H, k_opt)
...
if __name__ == "__main__":
    ...
    for M, N in zip(M_list, N_list):
        ...
        print(f"End of CV, optimal #neighbor = {k_opt}")
        
        temp = Parallel(n_jobs=n_jobs, verbose=1)(delayed(calRM)(M, N, d, S_0, K, mu, sigma, r, tau, T, h, H, L0, optionName, k_opt)
        ...
        print(f"Kernel Done for {optionName}, M={M}, N={N}, d={d}")

        result_df = pd.DataFrame(result_table)
        print(result_df)
        result_df.to_csv(f"{saveFolder}Kernel_{optionName}_{d}.csv")
